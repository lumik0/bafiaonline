...
export function normalizeAngle(angle: number): number {
    // // Приводим угол к диапазону [-2π, 2π]
    // angle = angle % (2 * global.Math.PI);
    
    // // Если угол больше π, преобразуем в отрицательный эквивалент
    // if(angle > global.Math.PI) {
    //     angle = 2 * global.Math.PI;
    // } else if(angle < -global.Math.PI) { // Если угол меньше -π, преобразуем в положительный эквивалент
    //     angle = 2 * global.Math.PI;
    // }
